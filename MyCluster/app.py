...
def get_pod_metrics():
    ...
    try:
        ...
        output = subprocess.check_output([
            ...
            "-o", "custom-columns=NAME:.metadata.name,NAMESPACE:.metadata.namespace,CONTAINER:.spec.containers[].name,CPU_REQUEST:.spec.containers[].resources.requests.cpu,MEMORY_REQUEST:.spec.containers[].resources.requests.memory,CPU_LIMIT:.spec.containers[].resources.limits.cpu,MEMORY_LIMIT:.spec.containers[].resources.limits.memory"
        ...
        for line in lines:
            ...
            pod_limits.append({
                ...
                "memory_request": parts[4],

                "cpu_limit": parts[5],

                "memory_limit": parts[6]
        ...
        print(f"Error running kubectl get pods: {e}")
...
def convert_memory_to_ki(memory_str):
    ...
    if memory_str == '<none>':
        ...
        return str(int(float(memory_str[:-2]) * 1024)) + ' Ki'

    elif memory_str.endswith('Gi'):

        return str(int(float(memory_str[:-2]) * 1024 * 1024)) + ' Ki'

    else:

        try:

            return str(int(float(memory_str))) + ' Ki'
...
def check_pods():
    ...
    for limit in pod_limits:
        ...
        for usage in pod_usage:
            ...
            if limit["pod_name"] == usage["pod_name"]:

                cpu_request = limit["cpu_request"]

                memory_request = limit["memory_request"]
                ...
                memory_request = convert_memory_to_ki(memory_request)

                memory_limit = convert_memory_to_ki(memory_limit)



                combined_data.append({

                    "pod_name": limit["pod_name"],
    ...
    return render_template('pod_metrics.html', pod_metrics=combined_data)
